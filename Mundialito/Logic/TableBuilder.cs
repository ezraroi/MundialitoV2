...
{
    private readonly ILogger<TableBuilder> _logger;     
    private readonly TournamentTimesUtils _tournamentTimesUtils;
    private readonly IDateTimeProvider _dateTimeProvider;
    
    public TableBuilder(ILogger<TableBuilder> logger, IDateTimeProvider dateTimeProvider, TournamentTimesUtils tournamentTimesUtils)
    {
        _logger = logger;
    ...
    public IEnumerable<UserModel> GetTable(IEnumerable<MundialitoUser> allUsers, IEnumerable<Bet> bets, IEnumerable<GeneralBet> generalBets ) {
        var users = allUsers.ToDictionary(user => user.Id, user => new UserModel(user));
        ...
        generalBets.ToList().ForEach(generalBet =>
            {
                users[generalBet.User.Id].SetGeneralBet(new GeneralBetViewModel(generalBet, _tournamentTimesUtils.GetGeneralBetsCloseTime()));
            });
